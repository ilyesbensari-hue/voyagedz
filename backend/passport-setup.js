const passport = require('passport');const GoogleStrategy = require('passport-google-oauth20').Strategy;const FacebookStrategy = require('passport-facebook').Strategy;const { getOne, query } = require('./db');passport.serializeUser((user, done) => {  done(null, user.id);});passport.deserializeUser(async (id, done) => {  try {    const user = await getOne('SELECT id, name, email, role, is_host FROM users WHERE id = $1', [id]);    done(null, user);  } catch (err) {    done(err, null);  }});// Google Strategypassport.use(new GoogleStrategy({    clientID: process.env.GOOGLE_CLIENT_ID || 'YOUR_GOOGLE_ID',    clientSecret: process.env.GOOGLE_CLIENT_SECRET || 'YOUR_GOOGLE_SECRET',    callbackURL: "/api/auth/google/callback"  },  async (accessToken, refreshToken, profile, done) => {    try {      // Check if user exists      let user = await getOne('SELECT * FROM users WHERE google_id = $1', [profile.id]);            if (!user) {        // Check by email        user = await getOne('SELECT * FROM users WHERE email = $1', [profile.emails[0].value]);                if (user) {          // Link account          await query('UPDATE users SET google_id = $1 WHERE id = $2', [profile.id, user.id]);
